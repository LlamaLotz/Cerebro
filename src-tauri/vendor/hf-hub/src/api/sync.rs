...
impl Api {
    ...
    fn metadata(&self, url: &str) -> Result<Metadata, ApiError> {
        ...
        let response = if is_redirection {
            // Prism patch: HuggingFace (via CloudFront) now returns a
            // *relative* `Location` header (e.g. `/api/resolve-cache/...`).
            // hf-hub 0.3.2 passed it straight to ureq, which rejects a
            // base-less URL with `RelativeUrlWithoutBase` and aborts the
            // whole model download. Resolve it against the endpoint first.
